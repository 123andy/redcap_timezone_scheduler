...
- `pages/admin.php` — project sidebar link "Timezone Scheduler Admin"; tabs for Appointments & Slots verification with repair action buttons. The **Instructions** tab is an admin dashboard: module overview, a configuration summary (`getConfigSummary()` — fields/events/slot DBs, enabled vs disabled), per-slot-DB usage stats (`getSlotDbStats()` → pure `summarizeSlots()` — total / available-upcoming / booked / cancelled / past + utilization %), and the copyable iCal subscription URL (`getIcalFeedUrl()`). Visible to users with Design rights.
